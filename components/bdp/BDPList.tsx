...
export function BDPList({ reports, companySettings, projectsData }: BDPListProps) {
    ...
    const [generatingMeasurement, setGeneratingMeasurement] = useState<string | null>(null)
    ...
    const handleMeasurementExport = async (projectName: string, projectReports: BDP[]) => {
        setGeneratingMeasurement(projectName)
        try {
            ...
            toast({
                ...
                description: `Relatório de medição da obra ${projectName} baixado com sucesso.`
            })
        } catch (error) {
            ...
            toast({
                variant: "destructive",
                title: "Erro",
                description: "Falha ao gerar o relatório de medição."
            })
        } finally {
            setGeneratingMeasurement(null)
        }
    }
