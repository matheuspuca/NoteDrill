...
export async function transferStock(data: { itemId: string, targetProjectId: string, quantity: number }) {
    ...
    if (sourceItem.quantity < data.quantity) return { error: "Quantidade insuficiente em estoque." }
    ...
    const { data: targetItem, error: targetError } = await supabase
        ...
        .eq("brand", sourceItem.brand || "") // Handle nulls if possible
        .maybeSingle()

    // 3. Perform Updates
    // A. Deduct from Source
    const newSourceQty = Number(sourceItem.quantity) - Number(data.quantity)
    ...
    if (targetItem) {
        // Update existing
        const newTargetQty = Number(targetItem.quantity) + Number(data.quantity)
        ...
        if (createError) return { error: "Erro ao criar item no destino." }
    }
